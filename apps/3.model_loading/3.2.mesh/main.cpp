...
void test_assimp(char const *path) {
...
int main() {
  ...
  while (!glfwWindowShouldClose(window)) {
    ...
    for (int i = 0; i < 10; i++) {
      ...
      // model = glm::rotate(model, glm::radians(time) * (10 + i), glm::vec3(0,
      // 1, 0));
      ...
      for (int i = 0; i < 4; i++) {
        ...
        shader_cube.set_vec3fv((light_name + "position").c_str(),
                               glm::value_ptr(PointLight_pos[i]));
        shader_cube.set_vec3f((light_name + "ambient").c_str(), 0.1f, 0.1f,
                              0.1f);
        shader_cube.set_vec3f((light_name + "diffuse").c_str(), 0.3f, 0.3f,
                              0.3f);
        shader_cube.set_vec3f((light_name + "specular").c_str(), 1.0f, 1.0f,
                              1.0f);
      ...
      // SpotLight
      shader_cube.set_vec3fv("spot_light.point_light.position",
                             glm::value_ptr(cam.camera_position()));
      ...
      shader_cube.set_vec3f("spot_light.point_light.specular", 1.0f, 1.0f,
                            1.0f);
      ...
      shader_cube.set_vec3fv("spot_light.direction",
                             glm::value_ptr(cam.camera_front()));
      shader_cube.set_float("spot_light.cutoff_inner",
                            glm::cos(glm::radians(12.5f)));
      shader_cube.set_float("spot_light.cutoff_outer",
