...
int main() {
  ...
  bound_shader.use();
  bound_shader.setInt("texture1", 0);
  while (!glfwWindowShouldClose(window)) {
    ...
    // Use bound shader
    bound_shader.use();
    bound_shader.setMat4fv("view", glm::value_ptr(view));
    bound_shader.setMat4fv("projection", glm::value_ptr(projection));
    // Cubes bound
    ...
    model = glm::mat4(1.0f);
    model = glm::translate(model, glm::vec3(-1.0f, -0.5f, -1.0f));
    bound_shader.setMat4fv("model", glm::value_ptr(model));
    glBindVertexArray(cube_VAO);
    glDrawArrays(GL_TRIANGLES, 0, 36);
    model = glm::mat4(1.0f);
    model = glm::translate(model, glm::vec3(2.0f, 0.0f, 0.0f));
